...
var ShopImpl = (function () {
    ...
    ShopImpl.prototype.addNewProduct = function (product) {
        if (!this.goods.has(product.id)) {
            this.goods.set(product.id, product);
            return true;
        }
        return false;
    };
    ...
    ShopImpl.prototype.listProductsByName = function (searchString) {
        const names = new Set();        //unique names
        const result = new Map();       //collection with results
        let count = 0;                  //iteration counter
        for (let item of this.goods.values()) {
            if (item.name.includes(searchString)) {         //search for matches
                if (names.has(item.name)) {
                    result.set(`${item.producer} - ${item.name}`, item.producer);
                    if (result.has(item.name)) {
                        const producer = result.get(item.name);
                        result.set(`${producer} - ${item.name}`, producer);
    ...
    ShopImpl.prototype.listProductsByProducer = function (searchString) {
        ...
        const result = [];
        for (let item of this.goods.values()) {
            if (item.producer.includes(searchString)) {         //search for matches
                result.push(item)
                count++;
            }
            if (count === 10) {
